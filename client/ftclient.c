...
int ftclient_help(int sock_data, int sock_con)
{
	size_t num_recvd;			// number of bytes received with recv()
	char buf[MAXSIZE];			// hold a filename received from server
	int tmp = 0;
...
{		
	...
	while (1) { // loop until user types quit
		...
		retcode = read_reply();		
		if (retcode == SERVER_CLOSED) {
			/* If command was quit, just exit */
			print_reply(SERVER_CLOSED);		
			break;
		}
		
		if (retcode == SERVER_CMD_FAILED) {
			...
			if (strcmp(cmd.code, "LIST") == 0) {
				...
				if (read_reply() == SERVER_REQ_NO_ACK) {
					print_reply(SERVER_REQ_NO_ACK);		
					close(data_sock);
					continue; 
				}
